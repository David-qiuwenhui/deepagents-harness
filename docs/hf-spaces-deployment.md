...
|------|------|------|
| 部署次数 | **无限制** | 随时推送代码触发重建 |
| 并发 Space 数 | 通常 5-10 个 | 账号级别，非单个 Space |
| 构建频率 | 无硬性限制 | 频繁推送可能触发临时限流（429） |

### 资源限制（Free Tier — Docker Space）

| 资源 | 配置 | 影响 |
|------|------|------|
| CPU | 2 vCPU | 足够运行 FastAPI + LLM API 调用 |
| 内存 | 16 GB | 充裕，我们的应用占用很小 |
| 磁盘 | 50 GB | 构建镜像 + 运行时存储 |
| 构建超时 | ~30 分钟 | 依赖多时可能不够，需优化 Dockerfile |

### 注意事项

- **冷启动**：空闲后容器休眠，首次访问需等待 30 秒 - 2 分钟唤醒
- **非持久化存储**：容器重启后文件丢失（我们的 session 存在内存中，重启后需重新登录）
- **磁盘用量**：Docker 镜像计入 50 GB 限额，注意 `.dockerignore` 排除无关文件
- **网络**：出站请求无限制，适合调用外部 API（如智谱 API）
...
|------|------|--------|
| Free | $0 | 当前方案，适合演示 |
| T4 Small | ~$0.40/h | GPU 加速、无冷启动 |
| CPU Basic | ~$0.03/h | 无冷启动、更多资源 |

> **结论**：对于内部演示场景，Free 套餐完全够用。主要限制是冷启动延迟，可通过定时 ping 或升级解决。
